    ...
    if (matchs.length === 0) {
      ...
      for (const [nomChampionnat, listeMatchs] of Object.entries(parChampionnat)) {
        ...
        for (const m of listeMatchs) {
          ...
          // ==========================================
          // DÉTECTION INTELLIGENTE DES SCORES DE MI-TEMPS
          // ==========================================
          let htDom = null;
          let htExt = null;

          // 1. Essai de lecture via des objets imbriqués (ex: m.scores.halftime.home)
          if (m.scores && m.scores.halftime) {
            htDom = m.scores.halftime.home;
            htExt = m.scores.halftime.away;
          } 
          // 2. Essai de lecture si c'est une chaîne brute (ex: "1-0")
          else if (m.halftime_score) { 
            const parties = String(m.halftime_score).split('-');
            if (parties.length === 2) {
              htDom = parseInt(parties[0], 10);
              htExt = parseInt(parties[1], 10);
            }
          }
          // 3. Balayage des clés de premier niveau possibles
          else {
            const clesPossiblesDom = ['home_ht', 'half_time_home', 'halftime_home', 'ht_home', 'first_half_home_score'];
            const clesPossiblesExt = ['away_ht', 'half_time_away', 'halftime_away', 'ht_away', 'first_half_away_score'];

            for (let i = 0; i < clesPossiblesDom.length; i++) {
              if (m[clesPossiblesDom[i]] !== undefined && m[clesPossiblesDom[i]] !== null && m[clesPossiblesDom[i]] !== '') {
                htDom = parseInt(m[clesPossiblesDom[i]], 10);
                break;
              }
            }
            for (let i = 0; i < clesPossiblesExt.length; i++) {
              if (m[clesPossiblesExt[i]] !== undefined && m[clesPossiblesExt[i]] !== null && m[clesPossiblesExt[i]] !== '') {
                htExt = parseInt(m[clesPossiblesExt[i]], 10);
                break;
              }
            }
          }

          // --- SECTION MI-TEMPS (AFFICHAGE SÉCURISÉ) ---
          const aUneMiTempsValide = (htDom !== null && !isNaN(htDom)) && (htExt !== null && !isNaN(htExt));

          if (aUneMiTempsValide) {
            const firstHalfStr = `${htDom}-${htExt}`;
            let secondHalfStr = "0-0";
            
            // Calculer la 2e mi-temps : (Score Final - Score Mi-temps)
            if (scoreFinal && scoreFinal.includes("-")) {
              const scores = scoreFinal.split("-").map(Number);
              if (scores.length === 2) {
                const domTotal = scores[0];
                const extTotal = scores[1];
                const dom2nd = Math.max(0, domTotal - htDom);
                const ext2nd = Math.max(0, extTotal - htExt);
                secondHalfStr = `${dom2nd}-${ext2nd}`;
              }
            }
            message += `   ➡️ 1st Half : ${firstHalfStr} | 2nd Half : ${secondHalfStr}\n`;
          } else {
            message += `   ➡️ 1st Half : - | 2nd Half : -\n`;
          }
