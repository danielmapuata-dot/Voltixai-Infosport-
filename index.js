...
// --------------------------
// FONCTION DE FORMATAGE DU MESSAGE (CORRIGÉE SANS UNDEFINED)
// --------------------------
function formaterPublication(listeMatchs) {
  ...
  for (const [ligue, matchs] of Object.entries(groupes)) {
    ...
    matchs.forEach(match => {
      // Résolution sécurisée du statut du match
      const statut = match.status || match.match_status || 'FT';
      
      // Résolution sécurisée des noms d'équipes (gère les formats directs et les sous-objets)
      const equipeDom = match.home_name || (match.home && match.home.name) || match.home_team || 'Équipe Domicile';
      const equipeExt = match.away_name || (match.away && match.away.name) || match.away_team || 'Équipe Extérieur';
      
      // Résolution sécurisée des scores
      const scoreDom = match.score_home !== undefined ? match.score_home : ((match.scores && match.scores.home_score) !== undefined ? match.scores.home_score : 0);
      const scoreExt = match.score_away !== undefined ? match.score_away : ((match.scores && match.scores.away_score) !== undefined ? match.scores.away_score : 0);
