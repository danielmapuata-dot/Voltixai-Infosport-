...
function formaterPublication(listeMatchs) {
  ...
  // En-tête du post
  let texte = `⚽🚩 LIVE SCORE ➜ ${heureGMT} - GMT\n\n`;

  // 2. Regrouper les matchs par Pays et par Ligue
  const groupes = {};
  listeMatchs.forEach(match => {
    const pays = match.country || 'International';
    const drapeau = match.country_logo || '🌍'; // Si l'API fournit un emoji ou logo, sinon globe par défaut
  ...
  for (const [ligue, matchs] of Object.entries(groupes)) {
    ...
    matchs.forEach(match => {
      ...
      // Extraction des statistiques de l'API (valeurs par défaut si manquantes)
      ...
      // Lignes de statistiques avec émojis alignés
    ...
    texte += `\n`; // Espace entre les compétitions
  }

  // 4. Légende fixe au bas du post (comme sur l'image)
  texte += `────────────────────\n`;
...
async function lancerRobot() {
  try {
    ...
    // 2. Formatage du texte dans le style ciblé
