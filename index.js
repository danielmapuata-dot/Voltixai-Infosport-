...
// -------------------------------------------------------------
// EXTRACTION ULTRA-ROBUSTE DES NOMS, SCORES ET MINUTES
// -------------------------------------------------------------
function extraireNomEquipe(match, type) {
  const cle = type; 
  if (typeof match[`${cle}_name`] === 'string') return match[`${cle}_name`];
  if (typeof match[`${cle}_team_name`] === 'string') return match[`${cle}_team_name`];
  if (typeof match[`match_${cle}_team_name`] === 'string') return match[`match_${cle}_team_name`];
  if (typeof match[`match_${cle}_name`] === 'string') return match[`match_${cle}_name`];
  if (typeof match[`team_${cle}_name`] === 'string') return match[`team_${cle}_name`];

  if (match[cle] && typeof match[cle] === 'object' && match[cle].name) return match[cle].name;
  if (match[`${cle}_team`] && typeof match[`${cle}_team`] === 'object' && match[`${cle}_team`].name) return match[`${cle}_team`].name;

  if (typeof match[cle] === 'string') return match[cle];
  if (typeof match[`${cle}_team`] === 'string') return match[`${cle}_team`];

  return type === 'home' ? 'Équipe Domicile' : 'Équipe Extérieur';
}

function extraireScore(match, type) {
  const cle = type; 
  const clesPossibles = [
    `${cle}_score`,
    `score_${cle}`,
  ...
  for (const k of clesPossibles) {
    if (match[k] !== undefined && match[k] !== null) return match[k];
  }

  if (match.scores) {
    const clesImbriquees = [`${cle}_score`, `score_${cle}`, cle];
    for (const k of clesImbriquees) {
      if (match.scores[k] !== undefined && match.scores[k] !== null) return match.scores[k];
    }
  }
...
// Nouvelle fonction pour récupérer proprement la minute (ex: "75'")
function extraireMinute(match) {
  const statutBrut = (match.status || match.match_status || '').toLowerCase();

  // Si le match est terminé
  if (statutBrut === 'finished' || statutBrut === 'ft' || statutBrut === 'ended') {
    return 'FT';
  }
  // Si c'est la mi-temps
  if (statutBrut === 'half-time' || statutBrut === 'ht' || statutBrut === 'mid-time') {
    return 'HT';
  }
  // Si le match n'a pas commencé
  if (statutBrut === 'not started' || statutBrut === 'ns') {
    return match.time || match.match_time || '00:00';
  }

  // On cherche le nombre de minutes (ex: match.minute ou match.current_minute)
  const minute = match.minute !== undefined ? match.minute : (match.current_minute !== undefined ? match.current_minute : null);
  
  if (minute !== null && minute !== '') {
    return `${minute}'`;
  }

  // Si l'API renvoie une chaîne dans le statut qui contient déjà un chiffre (ex: "70", "70'")
  const chiffresStatut = statutBrut.match(/\d+/);
  if (chiffresStatut) {
    return `${chiffresStatut[0]}'`;
  }

  // Repli par défaut si on est en direct mais sans minute chiffrée
  return 'Live';
}
...
function formaterPublication(listeMatchs) {
  ...
  // En-tête : VOLTIXAI LIVE
  let texte = `⚽🚩 VOLTIXAI LIVE ➜ ${heureGMT} - GMT\n\n`;

  // 2. Regrouper les matchs par Pays et par Ligue
  const groupes = {};
  listeMatchs.forEach(match => {
    const pays = match.country || (match.league && match.league.country) || 'International';
    const drapeau = match.country_logo || '🌍'; 
    const competition = match.league_name || (match.league && match.league.name) || 'Coupe';
  ...
  for (const [ligue, matchs] of Object.entries(groupes)) {
    ...
    matchs.forEach(match => {
      // Récupération de la minute corrigée !
      const statut = extraireMinute(match);
      
      // Récupération des clubs et scores
      const equipeDom = extraireNomEquipe(match, 'home');
      const equipeExt = extraireNomEquipe(match, 'away');
      const scoreDom = extraireScore(match, 'home');
      const scoreExt = extraireScore(match, 'away');

      // Ligne principale du match
      texte += `⚫ ${statut} | ${equipeDom} ${scoreDom}-${scoreExt} ${equipeExt}\n`;

      // Mi-temps (si disponibles)
      const miTemps1 = match.score_first_half || (match.scores && match.scores.half_time) || '0-0';
      const miTemps2 = match.score_second_half || (match.scores && match.scores.full_time) || '0-0';
